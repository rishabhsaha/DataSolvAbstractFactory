...
namespace Tg2_Dev_2
{
    ...
    public class AppController
    {
        ...
        public AppController(Application app)
        {
            ...
            var xaml = @"<contentpage xmlns='http://xamarin.com/schemas/2014/forms' xmlns:x='http://schemas.microsoft.com/winfx/2009/xaml'>
                        <label text = ""Test Label"" verticaloptions = ""Center"" horizontaloptions = ""Center""/>
                        </ contentpage > ";
            //{Binding Title, Mode=TwoWay}
            // Get dummy data from somewhere (i.e. VS)
            // var model = JsonModel.Parse(@"{""Title"": ""Bar""}");
            viewModel.Xaml = xaml;
            viewModel.Json = @"{""Title"": ""Bar""}";
            //viewModel.PropertyChanged += OnPropertyChanged;
            //***********************************
            var page = new ContentPage();
            
            page.LoadFromXaml(viewModel.Xaml);
            //page.BindingContext = JsonModel.Parse(viewModel.Json);
            //************************************************
